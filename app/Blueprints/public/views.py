...
def public_info():
    return jsonify(message="Public Information version 1.0.1 from LenDenPay", data = get_system_details()), 200
...
def callback():
    if request.method == 'GET':
        ...
        try:
            if not request.json:return jsonify(message="No data received in POST request"), 400
            print(f"Received POST data: {request.json}")
            return jsonify(message="Callback received via POST"), 200
